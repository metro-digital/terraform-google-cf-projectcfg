...
| skip_default_vpc_dns_logging_policy | When set to true, the module will not create a DNS policy to enable DNS logging.<br/>As DNS logging is mandatory, you should only disable the policy creation if you plan<br/>to implement your own policy for further adjustments. | `bool` | `true` | no |
