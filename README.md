...
| skip_default_vpc_dns_logging_policy | When set to true the module will not a DNS policy to enable DNS logging.<br/>DNS logging is mandatory, there you should only disable the policy creation if you plan to<br/>create your own policy for further adjustments. | `bool` | `true` | no |
