	...
	cat <<-END_OF_DOC
		...
		Options:
		  ...
		  -i          (optional) Stop after initial creation of Service Account, bucket and permissions.
		                         Bootstrap will sill create write to output directory, but will
...
OUTPUT_HINT=$(echo "The generated Terraform code will be written to '${OUTPUT_DIR}'.")
...
if [[ ! -d "${OUTPUT_DIR}" ]]; then
	read -p "$(echo "Output directory '${OUTPUT_DIR}' does not exist. Create it (y/n)? ")" -n 1 -r
	echo
	if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	if [ "$(ls -A "${OUTPUT_DIR}")" ]; then
		read -p "$(echo "Output directory '${OUTPUT_DIR}' is not empty. Continue anyway (y/n)? ")" -n 1 -r
		echo
		if [[ ! $REPLY =~ ^[Yy]$ ]]; then
			log_error "Aborted."
			exit 1
...
cat <<-EOF >"terraform/${GCP_PROJECT_ID}.tfvars"
...
terraform -chdir=terraform import \
	-var-file="${GCP_PROJECT_ID}.tfvars" \
